...
class sysadmins::common {
    ...
    $real_ssh_keys = parseyaml(inline_template('<%= @auth_keys.each{ |k,v| v["options"] = [v["options"] || []].flatten + [scope["sysadmins::ssh_keys_opts"] || []].flatten + ["environment=\"SYSADMIN_USER=#{k.gsub(/@.*/, "")}\""]}.to_yaml %>'))
